...
object SparkAPIs {
  ...
  def main(args: Array[String]): Unit = {
    ...
    AdaptiveSparkPlan isFinalPlan=false
    +- HashAggregate(keys=[], functions=[count(1)])
       +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=32]
          +- HashAggregate(keys=[], functions=[partial_count(1)])
             +- SerializeFromObject
                +- Scan[obj#1]
   ...
   */
    ...
    +- HashAggregate(keys=[], functions=[count(1)])
       +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=81]
          +- HashAggregate(keys=[], functions=[partial_count(1)])
             +- Project
                +- Range (1, 1000000000, step=1, splits=6)
    ...
    +- HashAggregate(keys=[], functions=[count(1)])
       +- Exchange SinglePartition, ENSURE_REQUIREMENTS, [plan_id=106]
          +- HashAggregate(keys=[], functions=[partial_count(1)])
             +- SerializeFromObject
                +- MapElements $line24.$read$$iw$$Lambda$4554/0x00007f8861295f10@379def3d, obj#34: bigint
                   +- DeserializeToObject static_invoke(java.lang.Long.valueOf(id#19L)), obj#33: java.lang.Long
                      +- Range (1, 1000000000, step=1, splits=6)
